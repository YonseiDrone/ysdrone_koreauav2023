    ...
    def image_cb(self, rgb_image, depth_image):
        ...
        try:
            if self.mission == 3:
                ...
                if xyxy is not None:
                    distance = depth_frame[int((xyxy[1] + xyxy[3])/2), int((xyxy[0] + xyxy[2])/2)] # (y, x)

                    pixel_center = np.array([int((xyxy[0] + xyxy[2])/2), int((xyxy[1] + xyxy[3])/2)]) #(x, y)
                    ...
                    # ====================FLU coordinate(forward-left-up)==================================
